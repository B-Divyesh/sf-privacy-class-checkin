...
Generated with the factory image model (`factory-image`, Azure OpenAI image generation) on 2026-08-27. Output is original for this product. Source PNG and prompt sidecar are retained under `assets/src/`; the optimized WebP is built from `frontend/src/assets/`. The footer discloses AI-assisted botanical artwork.

Repair note, 2026-09-06: the approved artwork was reused rather than regenerated. Its optimized WebP now enters Vite's fingerprinted asset pipeline under `frontend/src/assets/`. The 1200×630 social card is a centre crop of the same original source. The apple-touch icon is a hand-drawn leaf mark using the documented paper and fern tokens. The 404 page uses the same paper texture, serif scale, clipped button, and forest rules without loading JavaScript.
